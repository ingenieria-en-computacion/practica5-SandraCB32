...
Node *new_node(Data d) {
    ...
    n->data = d;
    n->next = NULL; // Inicializa el siguiente nodo como NULL
    return n;
}
...
void delete_node(Node* n) {
    if (n == NULL) {
        return; // No hace nada si el nodo es NULL
    }
    if (n->next == NULL) { // Solo elimina nodos cuyo enlace al siguiente es NULL
        free(n);
    }
}
